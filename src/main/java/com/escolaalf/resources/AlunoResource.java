...
public class AlunoResource {
	...
	@RequestMapping(value = "/verificar-alunos-aprovados", method = RequestMethod.GET)
	public ResponseEntity<?> verificarNotaAluno() {
		...
		for (int j = 0; j < alunos.size(); j++) {
			if (GabaritoResource.gabarito.getQuestoes().size() > 0
					&& GabaritoResource.gabarito.getQuestoes().size() == this.alunos.get(j).getProvaDoAluno().size()) {
				for (int i = 0; i < GabaritoResource.gabarito.getQuestoes().size(); i++) {
					if (this.alunos.get(j).getProvaDoAluno().get(i).equals(GabaritoResource.gabarito.getQuestoes().get(i).getAlternativa())) {
			...
			if (notaFinal >= 7) {
				aprovados.add(this.alunos.get(j).getNome());
			}
		}
	...
	@RequestMapping(value = "/verificar-nota-alunos", method = RequestMethod.GET)
